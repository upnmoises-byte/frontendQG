...
@Component({
  selector: 'app-catalogos-panel',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './catalogos-panel.component.html',
  styleUrl: './catalogos-panel.component.css',
...
export class CatalogosPanelComponent implements OnInit {
  ...
  guardarMaterial(): void {
    ...
    req.subscribe({
      next: () => {
        this.notify.success(this.modoEdicion ? 'Material actualizado' : 'Material registrado');
        this.cerrarModalMaterial();
        this.cargarMateriales();
      },
      error: (err) => this.notify.error(this.mensajeError(err, 'No se pudo guardar el material'))
    });
  }
  ...
  toggleMaterial(m: Material): void {
    ...
    this.materialService.cambiarEstado(m.id, !m.activo).subscribe({
      next: () => {
        this.notify.success(m.activo ? 'Material desactivado' : 'Material activado');
        this.cargarMateriales();
      },
  ...
  eliminarMaterial(m: Material): void {
    if (!m.id || !confirm(`¿Eliminar el material "${m.nombre}"?`)) {
      return;
    }
  ...
  guardarEspecial(): void {
    ...
    req.subscribe({
      next: () => {
        this.notify.success(this.modoEdicion ? 'Especial actualizado' : 'Especial registrado');
        this.cerrarModalEspecial();
        this.cargarEspeciales();
      },
      error: (err) => this.notify.error(this.mensajeError(err, 'No se pudo guardar el especial'))
    });
  }

  toggleEspecial(e: CatalogoEspecial): void {
    if (!e.id) {
      return;
    }
    this.catalogoEspecialService.cambiarEstado(e.id, !e.activo).subscribe({
      next: () => {
        this.notify.success(e.activo ? 'Especial desactivado' : 'Especial activado');
        this.cargarEspeciales();
      },
      error: (err) => this.notify.error(this.mensajeError(err, 'No se pudo cambiar el estado'))
    });
  ...
  eliminarEspecial(e: CatalogoEspecial): void {
    if (!e.id || !confirm(`¿Eliminar el especial "${e.nombre}"?`)) {
      return;
    }
    this.catalogoEspecialService.eliminar(e.id).subscribe({
      next: () => {
        this.notify.success('Especial eliminado');
        this.cargarEspeciales();
      },
